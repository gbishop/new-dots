...
for video, group in dots.groupby('video'):
    ...
    for dot in group.itertuples():
        ...
        yhi = min(f0.shape[0] - 1, int(math.ceil(yc + side))) + 1
        xlo = max(0, int(xc - side))
        xhi = min(f0.shape[1] - 1, int(math.ceil(xc + side))) + 1
        for y in range(ylo, yhi):
            for x in range(xlo, xhi):
                ...
                weight = min(1.0, (r - d) ** 2)
                # weight = (r - d) ** 2
                # label is 1 if inside the dot
                label = int(d <= r)
                # try some powers
                p = np.array([1, 2, 3])
...
# look at what is wrong with 198
